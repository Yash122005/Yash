    ...
    >
      <div className="aspect-video relative overflow-hidden">
        <video
          ...
          playsInline
          preload="metadata"
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 overflow-hidden"
        />
        {/* Image fallback ONLY */}
        {!project.video && (
          <img
      ...
      <div className="p-6">
        <div className="flex flex-wrap gap-2 mb-3">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-[10px] font-bold rounded bg-white/5 text-gray-400 border border-white/5 uppercase tracking-wider"
            >
              {tag}
            </span>
          ))}
      ...
      {/* Spotlight Effect */}
      <div
        className={`absolute -inset-px opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none bg-[radial-gradient(circle_at_var(--x)_var(--y),rgba(59,130,246,0.1),transparent_80%)]`}
      />
    </motion.div>
  );
};
